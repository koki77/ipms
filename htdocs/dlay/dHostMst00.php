<?php
	...
	class dHostMst00 extends fwDBase00
	{
		...
		protected $sqlu = "UPDATE host_mst set host_id = ?,history_no = ?,machine_id = ?,host_nm = ?,host_text = ?,host_type = ?,update_kb = ?,delflg = ?,update_userid = ?,update_usernm = ?,update_deptid = ?,update_deptnm = ?,update_date = now() WHERE host_id = ? and history_no = ?";
		...
		{
			$this->prmu = array($dao->getHostId(),$dao->getHistoryNo(),$dao->getMachineId(),$dao->getHostName(),$dao->getHostText(),$dao->getHostType(),$dao->getUpdateKb(),$dao->getDelFlg(),$dao->getUpdateUserId(),$dao->getUpdateUserName(),$dao->getUpdateDeptId(),$dao->getUpdateDeptName(),$dao->getHostId(),$dao->getHistoryNo());
		}
